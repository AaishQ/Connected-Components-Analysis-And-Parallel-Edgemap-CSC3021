...
public class PageRank {
	...
	//ASSIGNMENT 2 Q2 Time measurement begins here
	// Need to track edgemap time for Amdahl's Law calculation
	long loop_start_time = System.nanoTime();
	long total_edgemap_ns = 0;
	int edgemap_calls = 0;
	//ASSIGNMENT 2 Q2 Time measurement ends here

	while( iter < max_iter && delta > tol ) {
	    // Power iteration step.
	    // 1. Transfering weight over out-going links (summation part)
	    
		//ASSIGNMENT 2 Q2 Edgemap() Time measurement begins here
	    long edgemap_start = System.nanoTime();
	    context.edgemap( matrix, PRrelax );
	    long edgemap_time = System.nanoTime() - edgemap_start;
	    total_edgemap_ns += edgemap_time;
	    edgemap_calls++;
	  //ASSIGNMENT 2 Q2 Edgemap() Time measurement ends here
	...
	//ASSIGNMENT 2 Q2 Print Time measurement begins here
	// This is for answering Q2 part i - Amdahl's Law calculation
	long loop_total_ns = System.nanoTime() - loop_start_time;
	double loop_total_sec = loop_total_ns * 1e-9;
	double edgemap_total_sec = total_edgemap_ns * 1e-9;
	double sequential_sec = loop_total_sec - edgemap_total_sec;
	
	System.err.println("=== Q2 Timing Measurements ===");
	System.err.println("Total loop time (parallel + sequential): " + loop_total_sec + " s");
	System.err.println("Total edgemap time (parallel part): " + edgemap_total_sec + " s");
	System.err.println("Sequential part time (loop - edgemap): " + sequential_sec + " s");
	if (edgemap_calls > 0) {
	    System.err.println("Average edgemap time per call: " + (edgemap_total_sec / edgemap_calls) + " s");
	}
	System.err.println("=== End Q2 Measurements ===");
	//ASSIGNMENT 2 Q2 Print Time measurement ends here
