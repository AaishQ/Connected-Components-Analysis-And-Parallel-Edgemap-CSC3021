...
class Driver {
	...
	// ASSIGNMENT 2 Change Start
	// For Q4: We need to use ParallelContextSimple for parallel PageRank when using CSC format and multiple threads
	// Trying to follow the assignment requirements as best I understand
	if( algorithm.equalsIgnoreCase( "PR" ) && format.equalsIgnoreCase( "CSC" ) && num_threads > 1 ) {
	    ParallelContextHolder.set( new ParallelContextSimple(num_threads) );
	    System.err.println( "NOTE: Using ParallelContextSimple for Q4 parallel PageRank" );
	} else if( format.equalsIgnoreCase( "ICHOOSE" ) ) {
	    ParallelContextHolder.set( new ParallelContextSimple(num_threads) );
	} else {
	    ParallelContextHolder.set( new ParallelContextSingleThread() );
	}
	//  ASSIGNMENT 2 Change end
	...
	try {
		...
		// ASSIGNMENT 2 change
		// For Q2: We need to measure the  total time from start to end of Driver.main
		// So I'll just note here that total time includes reading+init+loop+writing
		// ASSIGNMENT 2 Change end
